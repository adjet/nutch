...
public class HttpRobotRulesParser extends RobotRulesParser {
	...
	protected boolean followRules = false;
	...
	public HttpRobotRulesParser(Configuration conf) {
		...
		followRules = conf.getBoolean("http.robots.followrules", false);
	}
		...
		if (robotRules == null) { // cache miss
			...
			try {
				...
				if (followRules) {
					...
					robotRules = EMPTY_RULES; // use default rules
					LOG.info("Ignore robots.txt for " + url + ": http.robots.followrules=false");
				}
			} catch (Throwable t) {
